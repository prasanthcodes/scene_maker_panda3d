...
class DropDownMenu(DirectObject):
  ...
  def __removePopupMenu(self):
      ...
      if menuEvents:
         menuEvent=list(menuEvents.keys())
         activeMenu=list(menuEvents[menuEvent[0]].values())[0][0].__self__
...
if __name__ == '__main__':
   ...
   modelsPath=[p for p in [getModelPath().getDirectory(i) for i in range(getModelPath().getNumDirectories())] if p.getBasename().find('models')>-1]
