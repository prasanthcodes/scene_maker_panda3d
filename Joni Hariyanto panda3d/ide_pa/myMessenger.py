...
class Messenger:
    ...
    def ignore(self, event, object):
        ...
        if Messenger.notify.getDebug():
            Messenger.notify.debug(repr(object) + '\n now ignoring: ' + repr(event))
        ...
        if acceptorDict and object in acceptorDict:
    ...
    def send(self, event, sentArgs=[]):
        ...
        if Messenger.notify.getDebug() and not self.quieting.get(event):
            assert Messenger.notify.debug(
                'sent event: ' + event + ' sentArgs: ' + repr(sentArgs))
        if __debug__:
            foundWatch=0
        ...
        if not acceptorDict:
            ...
            return
        for object in list(acceptorDict.keys()):
    ...
    def find(self, needle):
        ...
        for event in keys:
            if repr(event).find(needle) >= 0:
                print(self.__eventRepr(event), end=' ')
    ...
    def __eventRepr(self, event):
        ...
        for object, (method, extraArgs, persistent) in list(acceptorDict.items()):
    ...
    def __repr__(self):
        ...
        str += "="*64 + "\n"
        for object, eventDict in list(self.__objectEvents.items()):
            str += "%s:\n" % repr(object)
            for event in list(eventDict.keys()):
                str += "     %s\n" % repr(event)
