...
PRmodules = [k for k in list(sys.modules.keys()) if k.find(__name__)>-1]
...
if len(PRmodules)>1:
   ...
   from pandac.PandaModules import AnimControl, AudioSound, MovieTexture, NodePath, PandaSystem
   ...
   def pauseAnim(self,respectTag=1):
       if respectTag:
          part=self.getPart()
          for n in range(part.getNumNodes()):
       ...
       for t in taskMgr.getTasks(): # ordinary tasks
           if ( t and hasattr(t,taskFuncNameQuery(t)) and t.name.find(IDE_tasksName)!=0 and
              ...
              if mod.find('direct.interval')==0:
                 if not (func.__self__.__class__.__name__=='ActorInterval' and\
                         func.__self__.actor.hasNetTag('nopause')):
                    unneededTasks.append(t)
                    t.interval.pause()
              elif sys.modules[mod].__file__.find(directModulesDir)<0:
                 unneededTasks.append(t)
       ...
       for t in unneededTasks:
           if hasattr(t,'interval'): # it must be python-based intervals
              ...
              if hasattr(t,'uponDeath'):
                 ...
                 uponDeath=None
              if taskWakeT(t): # it's a doLater task
                 ...
                 # no need to pass appendTask, since if it's already true,
                 # the task is already appended to extraArgs
                 newTask=taskMgr.add( t.ORIG_call, t.name, priority=t.ORIG_priority,
                                   extraArgs=t.ORIG_extraArgs, uponDeath=uponDeath)
   #            newTask.extraArgs=t.ORIG_extraArgs
   #            newTask.id=t.ORIG_id
   #            del t
