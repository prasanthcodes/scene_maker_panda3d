...
def setup(parent=None,buttonThrower=None,taskNamePrefix='',startNow=True):
    ...
    '''
    global topParent,collButtonThrower,buttonMouseLoc,buttonCTrav,collTaskNamePrefix
...
#       print 'RELEASED'
      ...
      if not self.stayAlive:
         for c in self.acceptOnceCommand:
             c()
      if delay:
         taskMgr.doMethodLater(.15,self.__doButtonCmd,collTaskNamePrefix+'doingButtonCommand')
      else:
         self.__doButtonCmd()
...
class myDialog(DirectObject):
  ...
  def nextButton(self,delay=0):
      ...
      nextBtns=[b for b in self.buttonList[btnIdx+1:] if b.state!=3]
      if nextBtns:
  ...
  def prevButton(self,delay=0):
      ...
      prevBtns=[b for b in self.buttonList[:btnIdx] if b.state!=3]
      if prevBtns:
         # set the in-focus button back to ready state,
         # but don't remove it's mouse press event hook
         if BUTTON_IN:
